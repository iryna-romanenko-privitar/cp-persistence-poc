...
//    }
    ...
    public static void defineSchema(JanusGraph graph) throws ExecutionException, InterruptedException {
        graph.tx().rollback();
        JanusGraphManagement management = graph.openManagement();

        PropertyKey id = management.makePropertyKey("id").cardinality(Cardinality.SINGLE).dataType(String.class).make();
        PropertyKey name = management.makePropertyKey("name").cardinality(Cardinality.SINGLE).dataType(String.class).make();

        management.buildIndex("byUniqueId", Vertex.class).addKey(id, Mapping.STRING.asParameter()).buildMixedIndex("search");
        management.buildIndex("searchByName", Vertex.class).addKey(name, Mapping.TEXT.asParameter()).buildMixedIndex("search");
        management.commit();

        ManagementSystem.awaitGraphIndexStatus(graph, "byUniqueId").call();
        ManagementSystem.awaitGraphIndexStatus(graph, "searchByName").call();
    }

    public static void generateDatasets(GraphTraversalSource g, int low, int high) {
        for (int i = low; i < high; ++i) {
            g.addV("Dataset")
                    .property("id", UUID.randomUUID().toString())
                    .property("name", "Dataset_" + i)
                    .next();
        }
        g.tx().commit();
        System.out.println("Generated datasets " + low + " - " + high);
    }

    public static void deleteDatasets(GraphTraversalSource g) {
        g.V().hasLabel("Dataset").drop().iterate();
        g.tx().commit();
    }
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        JanusGraph graph = client.getInstance();
        GraphTraversalSource g = graph.traversal();

        defineSchema(graph);
        generateDatasets(g, 0, 50000);
//        generateDatasets(g, 10000, 20000);
//        deleteDatasets(g);

        Long count = g.V().hasLabel("Dataset").count().next();
        System.out.println("Number of datasets in database: " + count);

        Long total = graph.indexQuery("searchByName", "v.name:*").vertexTotals();
        System.out.println("Number of datasets in search index: " + total);
